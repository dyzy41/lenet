...
@ARTICLE{9961863,
  ...
  pages={21-32},
  pages={21-32},
  keywords={Transformers;Feature extraction;Remote sensing;Fuses;Convolution;Semantic segmentation;Visualization;Deep learning;Attention;change detection;remote sensing image;vision transformer},
  doi={10.1109/JSTARS.2022.3224081}}
https://github.com/SONGLEI-arch/ACABFNet
'''
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        l = int(math.sqrt(N))
        self.pos1 = nn.Parameter(torch.randn([1, l, C])).to(x.device)
        self.pos2 = nn.Parameter(torch.randn([1, l, C])).to(x.device)
        self.pos3 = nn.Parameter(torch.randn([1, self.num_heads, l, C // self.num_heads])).to(x.device)
        self.pos4 = nn.Parameter(torch.randn([1, self.num_heads, l, C // self.num_heads])).to(x.device)
...
class ACABFNet(nn.Module):
    def __init__(self, nclass=2, head=[4,8,16,32]):
        super(ACABFNet, self).__init__()
    ...
    #     y = imgs.clone()
        ...
        OUT = F.upsample(OUT, scale_factor=8, mode='bilinear', align_corners=True) + self.deconv5(cat)
        # ===================================================
        return [OUT]
